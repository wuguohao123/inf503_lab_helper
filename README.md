...
2.  如果脚本已正确安装并启用，你会在页面上看到一个自定义的 SQL Lab 辅助面板，通常位于页面的左侧。

#### 首次使用或数据库连接重置后

在您**首次**使用本脚本，或者当虚拟机中的数据库连接被重置时，您需要**手动**完成以下步骤，直到数据库成功连接：

*   根据 uCertify Lab 的指示，手动完成连接数据库的步骤。这通常包括打开 SQL Developer、输入连接信息（如用户名、密码、SID）并点击“Connect”。
*   当您成功连接到数据库后，您的界面应类似于下面的截图所示（@1.png）：
    *   ![首次连接数据库后界面](images/1.png)
*   **完成首次连接后，刷新浏览器页面。** 刷新后，您会回到初始状态，此时界面应类似于下面的截图所示（@2.png）：
    *   ![浏览器刷新后初始界面](images/2.png)

#### 后续使用

在您完成**首次数据库连接并刷新浏览器**之后，或者在**后续的 Lab 练习中**，如果数据库连接已经建立（或不需要重新建立），您就可以直接点击脚本面板上的 "🚀 一键全通关 (0-6)" 按钮。脚本会自动执行后续的所有步骤，直到任务完成。
